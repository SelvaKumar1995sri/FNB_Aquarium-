...
vi.mock("./AuthContext", () => ({
  useAuth: () => ({ isAuthenticated: mockIsCustomerAuthenticated }),
}));
...
describe("CartContext", () => {
  ...
  it("fetches the cart on mount when the customer is authenticated", async () => {
    apiClient.get.mockResolvedValueOnce({ data: EMPTY_CART });

    const { result } = renderHook(() => useCart(), { wrapper: CartProvider });

    await waitFor(() => expect(apiClient.get).toHaveBeenCalledWith("/cart/"));
  ...
  it("addItem posts to /cart/items/ and updates cart state", async () => {
    apiClient.get.mockResolvedValueOnce({ data: EMPTY_CART });
    apiClient.post.mockResolvedValueOnce({ data: CART_WITH_ONE_ITEM });
    ...
    expect(apiClient.post).toHaveBeenCalledWith("/cart/items/", { product: 5, quantity: 2 });
    expect(result.current.cart).toEqual(CART_WITH_ONE_ITEM);
    expect(result.current.itemCount).toBe(2);
  });

  it("updateItem patches the item and updates cart state", async () => {
    apiClient.get.mockResolvedValueOnce({ data: CART_WITH_ONE_ITEM });
    ...
    expect(apiClient.patch).toHaveBeenCalledWith("/cart/items/10/", { quantity: 4 });
    expect(result.current.cart.items[0].quantity).toBe(4);
  });

  it("removeItem deletes the item and updates cart state", async () => {
    apiClient.get.mockResolvedValueOnce({ data: CART_WITH_ONE_ITEM });
    apiClient.delete.mockResolvedValueOnce({ data: EMPTY_CART });
